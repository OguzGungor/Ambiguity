...
public class MapManager implements Locatable {
  ...
  /*
   * 0 = x 1 = y 2 = type 3 = ambushAxis(or any specification) 4 = ambushedPlayer
   */
  ...
  public boolean setEvent(Block[][] map, int playerNo, int eventID) {
    if (map[this.locatable.getX()][this.locatable.getY()].getEvent() == 0
        && map[this.locatable.getX()][this.locatable.getY()].getPlayerNo() == playerNo) {
  ...
  private Block[][] moveSteps(Block[][] tempMap) {
    if (noOfBlocks == moveCounter)
      ...
      if (tempMap[order[moveCounter][0]][order[moveCounter][1]].getEvent() == 1
          && tempMap[order[moveCounter][0]][order[moveCounter][1]].getPlayerNo() != conqueror)// capital
      {
        // to do remove player
         ...
         * defeatedPlayer = tempMap[x][y].getPlayerNo(); removeVersus(x,y,tempMap);}
         */
      /*
       * 0 = no event; 1 = capital+ 2 = mines+ 3 = ambush+ 4 = supply 5 = hill+ 6 =
       * resistance 7 = merchants 8 = river+
       */
      else if (tempMap[order[moveCounter][0]][order[moveCounter][1]].getEvent() == 2
          && tempMap[order[moveCounter][0]][order[moveCounter][1]].getPlayerNo() != conqueror
          && tempMap[order[moveCounter][0]][order[moveCounter][1]].getPlayerNo() != 0)// mines
      {
        tempMap[order[moveCounter][0]][order[moveCounter][1]].setEvent(0);
        return tempMap;
      } else if (tempMap[order[moveCounter][0]][order[moveCounter][1]].getEvent() == 3
          && tempMap[order[moveCounter][0]][order[moveCounter][1]].getPlayerNo() != 0
          && tempMap[order[moveCounter][0]][order[moveCounter][1]].getPlayerNo() != conqueror)// ambush
      {
      ...
      /*
       * else if (tempMap[order[moveCounter][0]][order[moveCounter][1]].getEvent() ==
       * 4)//supply {}
       */
      else if (tempMap[order[moveCounter][0]][order[moveCounter][1]].getEvent() == 6
          && tempMap[order[moveCounter][0]][order[moveCounter][1]].getEmbeddedPlayerID() == conqueror)// resistance
      {
        tempMap[order[moveCounter][0]][order[moveCounter][1]].setEvent(0);
        resistance = true;
      ...
      tempMap = executeOccupation(tempMap, conqueror, order[moveCounter][0], order[moveCounter][1], -1, -1,
          order[moveCounter][2], ambush, resistance);
      if (ambush && additionalEvent) {
        tempMap[order[moveCounter][0]][order[moveCounter][1]].setEvent(0);
        try {
          tempMap = executeOccupation(tempMap, conqueror, 2 * ambushX - order[moveCounter][0],
              2 * ambushY - order[moveCounter][1], -1, -1, order[moveCounter][2], ambush, resistance);
        } catch (Exception ex) {
        }
        try {
          tempMap = executeOccupation(tempMap, conqueror, 2 * ambushX - order[moveCounter][0], order[moveCounter][1],
              -1, -1, order[moveCounter][2], ambush, resistance);
        } catch (Exception ex) {
        }
        try {
          tempMap = executeOccupation(tempMap, conqueror, order[moveCounter][0], 2 * ambushY - order[moveCounter][1],
              -1, -1, order[moveCounter][2], ambush, resistance);
        } catch (Exception ex) {
        }
        try {
          if (Math.floor(Math.random() * 6) > 3 && !resistancePlaced) {
            ...
            tempMap[2 * ambushX - order[moveCounter][0]][2 * ambushY - order[moveCounter][1]]
                .setEmbeddedPlayerID(resistancePlayer);
      ...
      if (resistance) {
        ...
        try {
          tempMap = executeOccupation(tempMap, conqueror, order[moveCounter][0] + 1, order[moveCounter][1] + 1, -1, -1,
              order[moveCounter][2], ambush, resistance);
        } catch (Exception ex) {
        }
        try {
      ...
      // checkBoundary(order[moveCounter][0],order[moveCounter][1],tempMap)){
      // System.out.println("Fill Start : " + order[moveCounter][0] + " " +
      // order[moveCounter][1]);
      // tempMap = fill(tempMap,conqueror);
      // }
  ...
  public boolean checkMove(Block[][] map, int gameType, int playerNo) {
    ...
    if (gameType == 2) {
      ...
      // eventCounter = 0;
      conqueror = playerNo;
      tempMap = moveSteps(tempMap);
      for (int i = 0; i < order.length; i++) {
        if (secondInteraction && checkBoundary(order[i][0], order[i][1], tempMap)) {
          System.out.println("Fill Start : " + order[i][0] + "  " + order[i][1]);
          tempMap = fill(tempMap, conqueror);
        }
      }
      currMap = tempMap;
          ...
          if (figure[i][j].getFigureID() != 0) {
            try {
              if (gameType != 2) {
                ...
                tempMap[this.locatable.getX() + i - w][this.locatable.getY() + j - h]
                    .setFigureID(this.locatable.getSelectedFigureID());
              }
  ...
  private boolean checkEvents(Block[][] map, Block[][] figure, Block[][] tempMap, int playerNo) {
    ...
    /*
     * 1 = capital+ 2 = mines+ 3 = ambush+ 4 = supply+ 5 = hill+ 6 = resistance/ 7 =
     * merchants+ 8 = river+ eventsFaced[][]
     */
    ...
    for (int i = 0; i < noOfBlocks; i++) {
      try {
        if (map[order[i][0]][order[i][1]].getFigureID() != 0
            && map[order[i][0]][order[i][1]].getPlayerNo() == playerNo) {
          return false;
        ...
        // tempCount++;
        // }
        // else if(map[order[i][0]][order[i][1]].getEvent() == 2 &&
        // map[order[i][0]][order[i][1]].getPlayerNo() != 0 ){
        // temp[tempCount][0] = order[i][0];
        // temp[tempCount][1] = order[i][1];
        // temp[tempCount][2] = 2;
        // tempCount++;
        // }
        // else if(map[order[i][0]][order[i][1]].getEvent() == 3 &&
        // map[order[i][0]][order[i][1]].getPlayerNo() != 0 ){
        // temp[tempCount][0] = order[i][0];
        // temp[tempCount][1] = order[i][1];
        // temp[tempCount][2] = 3;
        // if( capitals[map[order[i][0]][order[i][1]].getPlayerNo() - 1][0] ==
        // capitals[conqueror -1][0] &&
        // capitals[map[order[i][0]][order[i][1]].getPlayerNo() - 1][1] !=
        // capitals[conqueror -1][1] )
        // temp[tempCount][3] = 1;
        // else if( capitals[map[order[i][0]][order[i][1]].getPlayerNo() - 1][0] !=
        // capitals[conqueror -1][0] &&
        // capitals[map[order[i][0]][order[i][1]].getPlayerNo() - 1][1] ==
        // capitals[conqueror -1][1] )
        // temp[tempCount][3] = 2;
        ...
        // tempCount++;
        // }
        // else if(map[order[i][0] - 1 ][order[i][1]].getEvent() == 4 && map[order[i][0]
        // - 1][order[i][1]].getPlayerNo() == playerNo||
        // map[order[i][0] + 1][order[i][1]].getEvent() == 4 && map[order[i][0] +
        // 1][order[i][1]].getPlayerNo() == playerNo ||
        // map[order[i][0]][order[i][1] - 1].getEvent() == 4 &&
        // map[order[i][0]][order[i][1] - 1].getPlayerNo() == playerNo||
        // map[order[i][0]][order[i][1] + 1].getEvent() == 4 &&
        // map[order[i][0]][order[i][1] + 1].getPlayerNo() == playerNo){
        // temp[tempCount][0] = order[i][0];
        // temp[tempCount][1] = order[i][1];
        ...
        // tempCount++;
        // }
        // else if(map[order[i][0]][order[i][1]].getEvent() == 7 &&
        // map[order[i][0]][order[i][1]].getPlayerNo() != 0){
        // temp[tempCount][0] = order[i][0];
        // temp[tempCount][1] = order[i][1];
        // temp[tempCount][2] = 7;
          ...
          try {
            if ((this.locatable.getX() + i - w + 1) < map.length) {
              ...
              if (map[this.locatable.getX() + i - w + 1][this.locatable.getY() + j - h].getPlayerNo() == playerNo) {// sa??na
                if (interacting) {
                  ...
                  fillPoint[0] = this.locatable.getX() + i - w;
                  fillPoint[1] = this.locatable.getY() + j - h;
                  fillPoint[2] = playerNo;

                } else
                  interacting = true;
                if (pathFound) {
                  if (map[this.locatable.getX() + i - w + 1][this.locatable.getY() + j - h].getNext()[0] == -1
                      && map[this.locatable.getX() + i - w + 1][this.locatable.getY() + j - h].getNext()[0] == -1)
                    disorder[count][2] = path;
                  else {
                    disorder[count][2] = noOfPaths[playerNo - 1] + 1;
                    noOfPaths[playerNo - 1]++;
                    path = noOfPaths[playerNo - 1];
                    System.out.println("here 527: " + path);
                    pathFound = true;
                  }
                } else {
                  if (map[this.locatable.getX() + i - w + 1][this.locatable.getY() + j - h].getPathNo() == 1) {
                    ...
                    noOfPaths[playerNo - 1]++;
                    path = noOfPaths[playerNo - 1];
                    System.out.println("here 536: " + path);
                    pathFound = true;
                  } else if (map[this.locatable.getX() + i - w + 1][this.locatable.getY() + j - h].getPathNo() != 1) {
                    if (map[this.locatable.getX() + i - w + 1][this.locatable.getY() + j - h].getNext()[0] == -1
                        && map[this.locatable.getX() + i - w + 1][this.locatable.getY() + j - h].getNext()[0] == -1)
                      disorder[count][2] = map[this.locatable.getX() + i - w + 1][this.locatable.getY() + j - h]
                          .getPathNo();
                    else {
                      disorder[count][2] = noOfPaths[playerNo - 1] + 1;
                      noOfPaths[playerNo - 1]++;
                      path = noOfPaths[playerNo - 1];
                      System.out.println("here 546: " + path);
                      pathFound = true;
                    }
                  }
                }
            ...
            if ((this.locatable.getX() + i - w - 1) >= 0) {
              if (map[this.locatable.getX() + i - w - 1][this.locatable.getY() + j - h].getPlayerNo() == playerNo) {// soluna
                ...
                if (pathFound) {
                  if (map[this.locatable.getX() + i - w - 1][this.locatable.getY() + j - h].getNext()[0] == -1
                      && map[this.locatable.getX() + i - w - 1][this.locatable.getY() + j - h].getNext()[1] == -1)
                    disorder[count][2] = path;
                  else {
                    disorder[count][2] = noOfPaths[playerNo - 1] + 1;
                    noOfPaths[playerNo - 1]++;
                    path = noOfPaths[playerNo - 1];
                    System.out.println("here 572: " + path);
                    pathFound = true;
                  }
                } else {
                  if (map[this.locatable.getX() + i - w - 1][this.locatable.getY() + j - h].getPathNo() == 1) {
                    ...
                    noOfPaths[playerNo - 1]++;
                    path = noOfPaths[playerNo - 1];
                    System.out.println("here 581: " + path);
                    pathFound = true;
                  } else if (map[this.locatable.getX() + i - w - 1][this.locatable.getY() + j - h].getPathNo() != 1) {
                    if (map[this.locatable.getX() + i - w - 1][this.locatable.getY() + j - h].getNext()[0] == -1
                        && map[this.locatable.getX() + i - w - 1][this.locatable.getY() + j - h].getNext()[1] == -1)
                      ...
                      noOfPaths[playerNo - 1]++;
                      path = noOfPaths[playerNo - 1];
                      System.out.println("here 591: " + path);
                      pathFound = true;
                    }
                  }
                }
            ...
            if ((this.locatable.getY() + j - h + 1) < map[0].length) {
              if (map[this.locatable.getX() + i - w][this.locatable.getY() + j - h + 1].getPlayerNo() == playerNo) {// alt?na
                if (interacting) {
                  ...
                  interacting = true;
                if (pathFound) {
                  if (map[this.locatable.getX() + i - w][this.locatable.getY() + j - h + 1].getNext()[0] == -1
                      && map[this.locatable.getX() + i - w][this.locatable.getY() + j - h + 1].getNext()[1] == -1)
                    disorder[count][2] = path;
                  else {
                    disorder[count][2] = noOfPaths[playerNo - 1] + 1;
                    noOfPaths[playerNo - 1]++;
                    path = noOfPaths[playerNo - 1];
                    System.out.println("here 616: " + path);
                    pathFound = true;
                  ...
                  if (map[this.locatable.getX() + i - w][this.locatable.getY() + j - h + 1].getPathNo() == 1) {
                    ...
                    noOfPaths[playerNo - 1]++;
                    path = noOfPaths[playerNo - 1];
                    System.out.println("here 626: " + path);
                    pathFound = true;
                  } else if (map[this.locatable.getX() + i - w][this.locatable.getY() + j - h + 1].getPathNo() != 1) {
                    if (map[this.locatable.getX() + i - w][this.locatable.getY() + j - h + 1].getNext()[0] == -1
                        && map[this.locatable.getX() + i - w][this.locatable.getY() + j - h + 1].getNext()[1] == -1)
                      disorder[count][2] = map[this.locatable.getX() + i - w][this.locatable.getY() + j - h + 1]
                          .getPathNo();
                    else {
                      disorder[count][2] = noOfPaths[playerNo - 1] + 1;
                      noOfPaths[playerNo - 1]++;
                      path = noOfPaths[playerNo - 1];
                      System.out.println("here 636: " + path);
                      pathFound = true;
              ...
              if (map[this.locatable.getX() + i - w][this.locatable.getY() + j - h - 1].getPlayerNo() == playerNo) {// �st�ne
                ...
                if (pathFound) {
                  if (map[this.locatable.getX() + i - w][this.locatable.getY() + j - h - 1].getNext()[0] == -1
                    ...
                    noOfPaths[playerNo - 1]++;
                    path = noOfPaths[playerNo - 1];
                    System.out.println("here 663: " + path);
                    pathFound = true;
                  }
                } else {
                  if (map[this.locatable.getX() + i - w][this.locatable.getY() + j - h - 1].getPathNo() == 1) {
                    ...
                    noOfPaths[playerNo - 1]++;
                    path = noOfPaths[playerNo - 1];
                    System.out.println("here 672: " + path);
                    pathFound = true;
                  } else if (map[this.locatable.getX() + i - w][this.locatable.getY() + j - h - 1].getPathNo() != 1) {
                    if (map[this.locatable.getX() + i - w][this.locatable.getY() + j - h - 1].getNext()[0] == -1
                        && map[this.locatable.getX() + i - w][this.locatable.getY() + j - h - 1].getNext()[1] == -1)
                      ...
                      noOfPaths[playerNo - 1]++;
                      path = noOfPaths[playerNo - 1];
                      System.out.println("here 682: " + path);
                      pathFound = true;
                    }
                  }
                }
  ...
  private void order(int playerNo) {
    for(int i = 0; i < disorder.length; i++) {
      for(int k = 0; k < disorder[0].length; k++) {
        System.out.print(disorder[i][k] + " ");
      }
      System.out.println();
    }
    int[] temp = new int[4];
            ...
            try {
              if (disorder[j][3] != 0) {
                if (((order[i - tempCounter][0] == disorder[j][0] - 1) && (order[i - tempCounter][1] == disorder[j][1]))
                    || ((order[i - tempCounter][0] == disorder[j][0] + 1)
                        && (order[i - tempCounter][1] == disorder[j][1]))
                    || ((order[i - tempCounter][1] == disorder[j][1] - 1)
                        && (order[i - tempCounter][0] == disorder[j][0]))
                    || ((order[i - tempCounter][1] == disorder[j][1] + 1)
                        && (order[i - tempCounter][0] == disorder[j][0]))) {
                  for (int k = 0; k < 4; k++)
                    temp[k] = disorder[j][k];
  ...
  private boolean checkBoundary(int x, int y, Block[][] tempMap) {
    try {
      // for(int i = 0; i < order.length; i++){
      /*
       * if(x+1 == order[i][0] && y == order[i][0]) return false; else
       */ if ((tempMap[x + 1][y].getPathNo() != tempMap[x][y].getPathNo() && tempMap[x + 1][y].getPathNo() != 0))
        return true;
      /*
       * else if(x == order[i][0] && y +1 == order[i][0] ) return false;
       */else if ((tempMap[x][y + 1].getPathNo() != tempMap[x][y].getPathNo() && tempMap[x][y + 1].getPathNo() != 0))
        return true;
      /*
       * else if(x - 1 == order[i][0] && y == order[i][0]) return false;
       */else if ((tempMap[x - 1][y].getPathNo() != tempMap[x][y].getPathNo() && tempMap[x - 1][y].getPathNo() != 0))
        return true;
      /*
       * else if(x == order[i][0] && y -1 == order[i][0]) return false;
       */else if ((tempMap[x][y - 1].getPathNo() != tempMap[x][y].getPathNo() && tempMap[x][y - 1].getPathNo() != 0))
        return true;
      // }
      // if((tempMap[x + 1][y].getPathNo() != tempMap[x][y].getPathNo() && tempMap[x +
      // 1][y].getPathNo() != 0)||
      // (tempMap[x][y + 1 ].getPathNo() != tempMap[x][y].getPathNo() && tempMap[x][y
      // + 1 ].getPathNo() != 0)||
      // (tempMap[x - 1][y].getPathNo() != tempMap[x][y].getPathNo() && tempMap[x -
      // 1][y].getPathNo() != 0) ||
      // (tempMap[x][y - 1].getPathNo() != tempMap[x][y].getPathNo() && tempMap[x][y -
      // 1].getPathNo() != 0 ))
      // return true;
  ...
  private Block[][] fill(Block[][] tempMap, int playerNo) {

    for (int i = -1; i < 2; i++) {
      for (int j = -1; j < 2; j++) {
        try {
          if (tempMap[fillPoint[0] + i][fillPoint[1] + j].getPlayerNo() != playerNo) {
            if (checkFill(tempMap, fillPoint[0] + i, fillPoint[1] + j, playerNo)) {
              System.out.println("Fill checked : " + (fillPoint[0] + i) + "  " + (fillPoint[1] + j));
              // for(int k = 0 ; k < 4 ; k++)
              // noOfPaths[k] = 0;
              return tempMap;
              //fillSteps(tempMap, fillPoint[0] + i, fillPoint[1] + j, playerNo);
            }
          }
        } catch (Exception ex) {
        }
  ...
  private boolean checkFill(Block[][] tempMap, int x, int y, int playerNo) {
    int xAxisCounter = 0;
    int yAxisCounter = 0;
    // boolean again = false;
    for (int i = 0; i < x; i++) {
      if (tempMap[i][y].getPlayerNo() == playerNo) {

        // if(!again){
        // System.out.println("xAxisPoints : " + i + " " + y);
        xAxisCounter = 1;
        // again = true;
        // }
      }
    ...
    for (int i = x; i < tempMap.length; i++) {
      if (tempMap[i][y].getPlayerNo() == playerNo) {

        if (xAxisCounter == 1) {
          System.out.println("Oguz1 : " + i);
          xAxisCounter = 2;
        }
    ...
    for (int i = 0; i < y; i++) {
      if (tempMap[x][i].getPlayerNo() == playerNo) {
        // if(!again){
        yAxisCounter = 1;
        // again = true;
        // System.out.println("yAxisPoints : " + x + " " + i);
        // }
        // }else{
        // again = false;
        //
    ...
    for (int i = y; i < tempMap[0].length; i++) {
      if (tempMap[x][i].getPlayerNo() == playerNo) {
        if (yAxisCounter == 1) {
          yAxisCounter = 2;
          System.out.println("Oguz2 :  " + i);
        }
      }
    }

    if (xAxisCounter == 2 && yAxisCounter == 2)
      return true;
    else
      return false;
  ...
  private Block[][] fillSteps(Block[][] tempMap, int x, int y, int playerNo) {

    System.out.println(" OGUZ : " + tempMap[x][y].getPlayerNo() + "    " + playerNo + "  " + x + "  " + y);
    if (tempMap[x][y].getPlayerNo() == playerNo)
      return tempMap;
    else {
      System.out.println("Oguz : " + x + " :: " + y);
      tempMap[x][y].setPlayer(playerNo);
      tempMap[x][y].setPathNo(123);
      try {
        tempMap = fillSteps(tempMap, x + 1, y, playerNo);
      } catch (Exception ex) {
      ...
      try {
        tempMap = fillSteps(tempMap, x, y + 1, playerNo);
      } catch (Exception ex) {
  ...
  private Block[][] executeOccupation(Block[][] tempMap, int playerNo, int x, int y, int nextX, int nextY, int pathNo,
      boolean ambush, boolean resistance) {

    if (tempMap[x][y].getEvent() == 5 || tempMap[x][y].getEvent() == 8)
      return tempMap;
    else if (tempMap[x][y].getEvent() == 1 && tempMap[x][y].getPlayerNo() != conqueror
        && tempMap[x][y].getPlayerNo() != 0)// capital
    {
      // to do remove player
      defeatedPlayer = tempMap[x][y].getPlayerNo();
      tempMap[x][y].setEvent(0);
      this.removeVersus(x, y, tempMap);
    } else if (tempMap[x][y].getEvent() == 2 && tempMap[x][y].getPlayerNo() != conqueror
        && tempMap[x][y].getPlayerNo() != 0)// mines
    {
      tempMap[x][y].setEvent(0);
      additionalEvent = false;
      return tempMap;
    } else if (tempMap[x][y].getEvent() == 3 && tempMap[x][y].getPlayerNo() != conqueror
        && tempMap[x][y].getPlayerNo() != 0)// ambush
    {
     ...
     * 4)//supply {}
     */
    else if (tempMap[x][y].getEvent() == 6 && tempMap[x][y].getEmbeddedPlayerID() == conqueror
        && tempMap[x][y].getPlayerNo() != 0)// resistance
    {
      tempMap[x][y].setEvent(0);
      resistance = true;
    ...
    if (pathNo == tempMap[lastlyInserted[conqueror - 1][0]][lastlyInserted[conqueror - 1][1]].getPathNo()
        || tempMap[lastlyInserted[conqueror - 1][0]][lastlyInserted[conqueror - 1][1]].getPathNo() == 1) {
  ...
  // tempMap = executeOccupation(tempMap,conqueror, int x,int y , -1,-1,
  // order[moveCounter][2]);
