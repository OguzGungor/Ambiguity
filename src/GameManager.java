...
public class GameManager {
    ...
    private ArrayList<Integer> rotateSymetryCount;

    public GameManager() {
        this.rotateSymetryCount = new ArrayList<Integer>();
        this.rotateSymetryCount.add(-1);
        round = 0;
        selectedEventID = -1;
...
//      System.out.println("players[" + i + "] = " + players[i]); 
    ...
    public boolean setEvent() {
        if (specifications[0] == 2) {
            ...
            int[] eventPos = {mapManager.locatable.getX(), mapManager.locatable.getY()};
            this.client.addEvent(eventID, eventPos);
            selectedEventID = -1;
            this.rotateSymetryCount.clear();
            this.rotateSymetryCount.add((-1));
            return true;
        }
...
//        }
    ...
    public boolean move() {
        ...
        int figureID = mapManager.locatable.getSelectedFigureID();
        //DataManager.getDataManagerInstance
        //GameObjectManager.GetGameObjectManagerInstance.getFigures();
        if (specifications[0] == 2) {
            int[] figurePos = {mapManager.locatable.getX(), mapManager.locatable.getY()};
            int[] rotateSymetry = new int [this.rotateSymetryCount.size()];
            for (int i = 0; i < this.rotateSymetryCount.size(); i++) {
                rotateSymetry[i] = this.rotateSymetryCount.get(i);
            }
            int eventID = 0;
            int[] eventPos = {0, 0};
            this.client.move(figureID, figurePos, rotateSymetry, eventID, eventPos);
            this.rotateSymetryCount.clear();
            this.rotateSymetryCount.add(-1);
            this.dataManager.getFigure(selectedFigureID).resetShape();
            return true;
        }
    ...
    public void setCurrentMap(Block[][] currMap) {// size of the array?
    ...
    public void rotate(boolean dir, boolean sym) {
        ...
        val = dir ? 0 : val;
        val = sym ? 1 : val;
        this.rotateSymetryCount.add(this.rotateSymetryCount.size(), val);
    }
    ...
    public void initializeClient(MqttCallback callback) {
        this.client = Client.getInstance(callback);
    }
    ...
    public void enterLoby(int index) {
        this.client.enterLobby(index);
    }
    ...
    public void readyLoby() {
        this.client.readyLobby();
    }

    public void unreadyLoby() {
        this.client.unreadyLobby();
    }
    ...
    public String[][] runCommand(String topic, String message) {
        return this.client.runCommand(topic, message);
    }

    public Block[][] parseMap(String message) {
        return null;
    }
    ...
    public void getClientNames() {
        this.client.getClientNames();
    }
    
    public void setPlayerName(String val) {
        this.client.setPlayerName(val);
    }
}
